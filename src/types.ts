...
export interface Transaction {
  ...
  userId?: string; // Рахунок: на кого з користувачів CRM записана операція (id користувача). Якщо не задано — «Без рахунку».
  // Якщо задано — це операція конвертації валюти всередині одного рахунку:
  // з рахунку списується `amount` у валюті `currency` і додається `conversion.toAmount`
  // у валюті `conversion.toCurrency` за курсом самого користувача. Такі операції
  // рухають баланси рахунку між валютами, але НЕ рахуються як дохід/витрата у
  // підсумках, звітах і на дашборді (це ті самі гроші, лише в іншій валюті).
  conversion?: {
    toAmount: number;
    toCurrency: string;
  };
...
export interface GgselWatchItem {
  ...
  groupTitleOverride?: string; // власна назва товару-групи (для об'єднаної картки з кількома номіналами), однакова на всіх номіналах групи
  addedAt: string; // ISO
}
